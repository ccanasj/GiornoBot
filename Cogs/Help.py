...
class Help(commands.Cog):
    ...
    @help.command(aliases = ['ZH'])
    async def Zahando(self,ctx):
        embed = discord.Embed(title = '**__ZaHando(Seguramente lo cambie xd)__**', description = 'Con este comando puedes poner efectos a tu foto de perfil',color = discord.Colour.blue())
        embed.add_field(name= '**__Sintaxis__**' , value='$ZaHando [Numero del efecto] <Mencion> \n$ZH [Numero del efecto] <Mencion>')
        embed.set_footer(text = 'De momento hay 7 empezando desde el 1, igual creo que lo voy a cambiar xd')
        await ctx.send(embed = embed)

    @help.command(aliases = ['SA'])
    async def StandAvatar(self,ctx):
        embed = discord.Embed(title = '**__StandAvatar__**', description = 'Manda la imagen de tu perfil o del usuario que menciones',color = discord.Colour.dark_orange())
        embed.add_field(name= '**__Sintaxis__**' , value='$StandAvatar <Mencion> <Mencion>  \n$SA <Mencion> <Mencion> ')
        embed.set_footer(text = 'Se puede mencionar hasta 2 usuarios, tengo que hacer este comando mas interesante xd')
        await ctx.send(embed = embed)
